...
- [ ] Add recovery behavior for automation event-bus lag so missed triggers are handled intentionally instead of only logging a warning.
...
- [ ] Add failure-mode tests for adapter outages, persistence lag, automation lag recovery, slow Lua execution, and shutdown behavior.

## P1 History, Audit, And Time-Based State

- [ ] Implement time-based device state history instead of current-state-only persistence.
- [ ] Persist attribute-level history with timestamps so state changes can be queried over time.
- [ ] Decide retention rules for history data and make them configurable.
- [ ] Add history APIs for device timelines, attribute timelines, and time-range queries.
...
- [ ] Add migrations/versioning for persistence schema changes instead of relying only on `CREATE TABLE IF NOT EXISTS`.
